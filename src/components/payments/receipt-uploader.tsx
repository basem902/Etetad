...
export function ReceiptUploader({ name, required, disabled }: Props) {
  ...
  function handleChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (picked.size > MAX_RECEIPT_SIZE) {
      setError('الحجم يجب أن يكون أقل من 5 ميجا')
      e.target.value = ''
      setFile(null)
      setPreviewUrl(null)
      return
    }
    if (!(ALLOWED_RECEIPT_MIMES as readonly string[]).includes(picked.type)) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        الأنواع المسموحة: JPG, PNG, WebP, PDF — الحد الأقصى 5 ميجا.
      </p>
    </div>
  )
}
